...
/** Private, account-to-account sharing for individual photos and videos. */
export function AssetShareDialog({
  assetIds,
  open,
  onClose,
}: {
  ...
  const share = useMutation({
    mutationFn: async () => api.post('/assets/share', { ids: assetIds, userIds: [...selected] }),
    onSuccess: () => {
      setSelected(new Set());
      setError(null);
  ...
  return (
    <Dialog
      ...
      width="sm"
      title={`Share ${count === 1 ? 'photo' : `${count} photos`}`}
      description="Only the accounts you choose can view these files. They cannot edit, move, or delete them."
    >
      <div className="space-y-4">
        ...
        >
          {share.isPending ? 'Sharing…' : 'Share privately'}
        </Button>
      </div>
    </Dialog>
  );
